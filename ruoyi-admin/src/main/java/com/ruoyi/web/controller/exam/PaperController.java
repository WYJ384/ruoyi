...
//		根据试卷ID查询考试
        Integer examByExamPaperId = examService.getExamByExamPaperId(ids);
        if (examByExamPaperId == 0) {
            ajaxResult = toAjax(paperService.deletePaperByIds(ids));
        } else {
            ajaxResult=AjaxResult.error("该试卷已使用，不可以删除！");
        }
        return ajaxResult;
    }

...
public class SingleChoiceController extends BaseController {
     ...
     * 新增保存单选题
     */
    @Log(title = "案例评分", businessType = BusinessType.INSERT)
    @PostMapping("/add")
    @ResponseBody
    public AjaxResult addSave(SingleChoice singleChoice) {
        String choiceA = singleChoice.getChoiceA();
        if (StringUtils.isNotEmpty(choiceA)) {
            String[] arrScores = choiceA.split(",");
            singleChoice.setChoiceB(arrScores[0]);
            singleChoice.setChoiceC(arrScores[1]);
            singleChoice.setChoiceD(arrScores[2]);
            singleChoice.setChoiceE(arrScores[3]);
            Double sum=Double.valueOf(arrScores[0])+Double.valueOf(arrScores[1])+Double.valueOf(arrScores[2])+Double.valueOf(arrScores[3]);
            singleChoice.setChoiceF(sum+"");
            singleChoice.setRemark(ShiroUtils.getSysUser().getUserName());
        }
